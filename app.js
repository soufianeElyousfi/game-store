(() => {
  ...
  function makeOnlineCard(g) {
    const card = document.createElement('div');
    card.className = 'online-card';
    const tagsHtml = (g.tags || []).slice(0, 2).map(tag =>
      `<span class="online-card-tag">${tag}</span>`
    ).join('');
    card.innerHTML = `
      ${g.thumb
        ? `<img class="online-card-thumb" src="${g.thumb}" alt="${g.name}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">`
        : ''}
      <div class="online-card-thumb-placeholder" style="${g.thumb ? 'display:none' : ''}">🎮</div>
      <div class="online-card-body">
        <div class="online-card-name">${g.name}</div>
        <div class="online-card-tags">${tagsHtml || '<span class="online-card-tag">HTML5</span>'}</div>
        <div class="online-play-badge"><span class="material-icons">play_circle</span> العب الآن</div>
      </div>`;
  ...
  function openGamePlayer(g) {
    $('gamePlayerTitle').textContent = g.name;
    $('gameFrame').src = g.embedUrl;
    $('gamePlayerOverlay').classList.add('open');
    document.body.style.overflow = 'hidden';
  }

  function closeGamePlayer() {
    $('gameFrame').src = '';
    $('gamePlayerOverlay').classList.remove('open');
    document.body.style.overflow = '';
  }

  async function loadOnlineGames(reset = false) {
    if (onlineLoading || onlineExhausted) return;
    onlineLoading = true;
    ...
    try {
      ...
      if (games.length === 0) {
        ...
        games.forEach(g => $('onlineGrid').appendChild(makeOnlineCard(g)));
        onlinePage++;
      }
    } catch(e) { /* ignore */ }
  ...
  const onlineObserver = new IntersectionObserver(entries => {
    if (entries[0].isIntersecting) loadOnlineGames();
  }, { rootMargin: '200px' });
  onlineObserver.observe($('onlineSentinel'));

  function openOnlinePage() {
    $('onlineOverlay').classList.add('open');
    document.body.style.overflow = 'hidden';
    if ($('onlineGrid').children.length === 0) loadOnlineGames(true);
  }

  function closeOnlinePage() {
    $('onlineOverlay').classList.remove('open');
    document.body.style.overflow = '';
  }

  $('onlineBtn').addEventListener('click', openOnlinePage);
  $('onlineBack').addEventListener('click', closeOnlinePage);

  $('gamePlayerBack').addEventListener('click', closeGamePlayer);
  $('gamePlayerFullscreen').addEventListener('click', () => {
    const frame = $('gameFrame');
    if (frame.requestFullscreen) frame.requestFullscreen();
    else if (frame.webkitRequestFullscreen) frame.webkitRequestFullscreen();
  });

  $('onlineSearch').addEventListener('input', e => {
    clearTimeout(onlineSearchTimeout);
    onlineSearchTimeout = setTimeout(() => {
      onlineQuery = e.target.value.trim();
      loadOnlineGames(true);
    }, 500);
  });
